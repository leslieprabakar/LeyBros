    ...
    <div>
      ...
      <section id="services" className="mx-auto max-w-[1280px] px-4 sm:px-6 py-12">
        <SectionHeader
          ...
          action={<Link href="/contact" className="hidden sm:inline-flex rounded-full border border-[#C9A86A]/20 px-5 py-2 text-xs font-semibold text-[#E8D5B5] hover:bg-[#C9A86A]/10">LET&apos;S TALK →</Link>}
        />
        <div className="mt-8 grid gap-4 md:grid-cols-3 lg:grid-cols-6">
          {[
            ...
            <div key={c.t} className="rounded-[16px] border border-[#C9A86A]/12 bg-[#10201A]/60 p-5 hover:border-[#C9A86A]/20 transition">
              <div className="h-8 w-8 rounded-full bg-[#C9A86A]/10 border border-[#C9A86A]/20 grid place-items-center text-[#C9A86A] text-sm">{c.icon}</div>
              <div className="mt-3 text-sm font-semibold text-white">{c.t}</div>
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 py-8">
        <div className="flex items-end justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 text-[11px] tracking-[0.18em] text-[#C9A86A]"><span>✦</span> SELECTED WORK</div>
            <h2 className="mt-2 font-display text-[26px] sm:text-[32px] font-medium text-white">A selection of my recent work</h2>
          </div>
          <Link href="/create" className="hidden sm:inline-flex text-xs tracking-wide text-[#C9A86A] hover:text-[#E8D5B5]">EXPLORE ALL PROJECTS →</Link>
        </div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {projects.slice(0,6).map(p=> <ProjectCard key={p.slug} p={p} />)}
        </div>
        <div className="mt-4 flex flex-wrap gap-2">
          <Link href="/build" className="rounded-full bg-[#C9A86A] px-5 py-2 text-xs font-semibold text-[#081410] hover:bg-[#D4B78A]">View BUILD case studies</Link>
          <Link href="/create" className="rounded-full border border-[#C9A86A]/20 px-5 py-2 text-xs font-semibold text-[#E8D5B5] hover:bg-[#C9A86A]/10">View CREATE visuals</Link>
        </div>
        <p className="mt-3 text-[11px] text-zinc-500">Tip: drop images into <code className="px-1 py-0.5 bg-white/10 rounded">/public/portfolio/</code> and flip <code>status: placeholder → published</code> in <code>src/data/projects.ts</code></p>
      </section>
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 py-6">
        <div className="rounded-[16px] border border-[#C9A86A]/10 bg-[#10201A]/40 p-6">
          <div className="text-center text-[11px] tracking-[0.18em] text-[#C9A86A]">SKILLS & TOOLS — TECHNOLOGIES I WORK WITH</div>
          <div className="mt-4 flex flex-wrap justify-center gap-2 text-xs">
            {["Figma","Adobe XD","Photoshop","Illustrator","HTML5","CSS3","JavaScript","React.js","Tailwind CSS","Next.js","Supabase","Git & GitHub","VS Code"].map(t=>(
              <span key={t} className="rounded-full border border-[#C9A86A]/10 bg-[#081410] px-3 py-1.5 text-zinc-300">{t}</span>
            ))}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 py-10">
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-2xl border border-[#C9A86A]/12 bg-[#10201A]/60 p-6 sm:p-8 relative overflow-hidden">
            ...
            <div className="text-[11px] tracking-[0.18em] text-[#C9A86A]">ABOUT ME</div>
            <h3 className="mt-2 font-display text-xl sm:text-2xl font-medium text-white leading-tight">Passionate about design,<br /><span className="text-[#C9A86A]">driven by purpose.</span></h3>
            ...
            <div className="mt-4 grid grid-cols-3 gap-3 text-xs">
              <div className="rounded-xl bg-[#081410] border border-[#C9A86A]/10 p-3"><div className="text-[#C9A86A]">✓ Pixel-Perfect</div><div className="text-zinc-500 mt-1">Clean, modern and user-focused</div></div>
              <div className="rounded-xl bg-[#081410] border border-[#C9A86A]/10 p-3"><div className="text-[#C9A86A]">✓ Clean Code</div><div className="text-zinc-500 mt-1">Scalable, semantic</div></div>
              <div className="rounded-xl bg-[#081410] border border-[#C9A86A]/10 p-3"><div className="text-[#C9A86A]">✓ User-Centered</div><div className="text-zinc-500 mt-1">Empathy & focus</div></div>
            </div>
            <Link href="/about" className="mt-6 inline-flex rounded-full bg-[#C9A86A] px-6 py-2.5 text-xs font-semibold text-[#081410] hover:bg-[#D4B78A]">MORE ABOUT ME →</Link>
          </div>
          <div className="rounded-2xl border border-[#C9A86A]/12 bg-gradient-to-b from-[#10201A] to-[#0d1a14] p-6 sm:p-8 relative overflow-hidden">
            ...
            <div className="text-[11px] tracking-[0.18em] text-[#C9A86A]">TIMELINE — NO INVENTED DATES</div>
            <div className="mt-6 space-y-5">
              {[
                ...
                <div key={org} className="flex gap-3">
                  <div className="h-2 w-2 mt-2 rounded-full bg-[#C9A86A] shrink-0" />
            ...
            <p className="mt-4 font-script text-lg text-[#C9A86A]/70">Leslie</p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 py-8">
        <div className="text-center">
          ...
          <h2 className="mt-2 font-display text-2xl font-medium text-white">How I bring ideas to life</h2>
        </div>
          ...
          <div className="grid gap-4 md:grid-cols-3 lg:grid-cols-6">
            {[
              ...
              <div key={n} className="text-center">
                <div className="mx-auto h-10 w-10 rounded-full bg-[#C9A86A] text-[#081410] grid place-items-center text-xs font-bold">{n}</div>
                <div className="mt-3 text-sm font-semibold text-white">{t}</div>
                <p className="mt-1 text-xs leading-5 text-zinc-400">{d}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 py-12">
        ...
        <div className="mt-2 text-[11px] text-zinc-500">Affordable by design — compare: Starter less than a month&apos;s ad spend.</div>
        ...
        <div className="mt-6 flex flex-wrap gap-2 justify-center text-[11px]">
          <span className="rounded-full border border-[#C9A86A]/15 px-3 py-1 text-zinc-400">Add-on: SEO Retainer ₹1,999/mo</span>
          <span className="rounded-full border border-[#C9A86A]/15 px-3 py-1 text-zinc-400">Maintenance ₹999/mo</span>
          <span className="rounded-full border border-[#C9A86A]/15 px-3 py-1 text-zinc-400">Domain & hosting guidance included</span>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 py-8">
        <div className="rounded-[24px] border border-amber-500/15 bg-gradient-to-b from-amber-500/[0.06] to-transparent p-6 sm:p-8">
          <div className="flex flex-wrap gap-2">
            <span className="rounded-full border border-amber-500/25 bg-amber-500/15 px-3 py-1 text-[11px] tracking-wide text-amber-200">STOCK-MARKET — SEPARATE</span>
            <span className="rounded-full border border-[#C9A86A]/10 px-3 py-1 text-[11px] text-zinc-400">Education only</span>
            <span className="rounded-full border border-[#C9A86A]/10 px-3 py-1 text-[11px] text-zinc-400">Not SEBI-registered</span>
          </div>
          <h2 className="mt-4 font-display text-[24px] sm:text-[28px] font-medium text-white">Stock-market knowledge sharing — practical notes, not calls.</h2>
          ...
          <div className="mt-6 rounded-xl border border-amber-500/15 bg-black/30 p-4 text-xs leading-5 text-zinc-400">
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            ...
            <Link href="/contact?intent=observe" className="rounded-full border border-amber-500/25 px-6 py-3 text-xs font-semibold text-amber-200 hover:bg-amber-500/10">Enquire — Knowledge Program</Link>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1280px] px-4 sm:px-6 py-8">
        <div className="rounded-[24px] border border-[#C9A86A]/15 bg-gradient-to-r from-[#10201A] to-[#0f241c] p-8 text-center">
          <div className="text-[11px] tracking-[0.18em] text-[#C9A86A]">LET&apos;S WORK TOGETHER</div>
          <h2 className="mt-2 font-display text-2xl sm:text-3xl font-medium text-white">Have a project in mind?<br /><span className="font-script text-[#C9A86A] font-normal">Let&apos;s create something amazing.</span></h2>
          <p className="mt-3 text-sm text-zinc-400">I&apos;m available for freelance and collaborations. Let&apos;s talk about your idea.</p>
          <div className="mt-6 flex flex-wrap gap-3 justify-center">
            <Link href="/contact" className="rounded-full bg-[#C9A86A] px-8 py-3 text-xs font-semibold text-[#081410] hover:bg-[#D4B78A]">SEND MESSAGE ✉</Link>
            <Link href="/contact" className="rounded-full border border-[#C9A86A]/25 px-8 py-3 text-xs font-semibold text-[#E8D5B5] hover:bg-[#C9A86A]/10">BOOK A CALL</Link>
          </div>
        </div>
      </section>
    </div>
